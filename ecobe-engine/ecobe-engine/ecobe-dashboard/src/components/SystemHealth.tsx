...
import { Activity, Clock, Database, Loader2, Zap } from 'lucide-react'
...
export function SystemHealth() {
  ...
  const signalSuccess =
    metrics?.providerSignals?.successRate != null
      ? (metrics.providerSignals.successRate * 100).toFixed(1)
      : null
    ...
    <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <MetricCard
          ...
          value={signalSuccess != null ? `${signalSuccess}%` : '-'}
          sub="Live provider mesh"
          color="text-teal-400"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          sub="oldest signal"
          color={dataFreshness != null && dataFreshness > 600 ? 'text-red-400' : 'text-orange-400'}
        />
      </div>
      ...
      {metrics && (
        <div className="border-t border-slate-800 pt-4 space-y-2">
          ...
          <div className="flex justify-between text-xs">
            <span className="text-slate-400">CO2 avoided / 1k requests</span>
          ...
          <div className="flex justify-between text-xs">
            ...
            <span className="text-emerald-400">
              {metrics.greenRouteRate != null ? `${(metrics.greenRouteRate * 100).toFixed(1)}%` : '-'}
            </span>
          </div>
          <div className="flex justify-between text-xs">
            ...
            >
              {metrics.providerSignals?.failureCount ?? 0}
            </span>
          </div>
        </div>
      )}
