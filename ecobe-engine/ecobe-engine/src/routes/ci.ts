...
async function getPersistedLatencyWindow(limit = 250) {
  ...
  const hotPathTotalMs: number[] = []
  const hotPathComputeMs: number[] = []
  ...
  for (const decision of recentDecisions) {
    ...
    if (isHotPath && isFiniteLatency(latency?.total)) {
      hotPathTotalMs.push(latency.total)
    }
    if (isHotPath && isFiniteLatency(latency?.compute)) {
      hotPathComputeMs.push(latency.compute)
    }
  }
  ...
  return {
    ...
    hotPathTotalMs: hotPathTotalMs.reverse(),
    hotPathComputeMs: hotPathComputeMs.reverse(),
  }
}
...
router.get('/slo', async (_req, res) => {
  ...
  if (totalSamples.length === 0 || computeSamples.length === 0) {
    ...
    if (totalSamples.length === 0 && persisted.hotPathTotalMs.length > 0) {
      totalSamples = persisted.hotPathTotalMs
    }
    if (computeSamples.length === 0 && persisted.hotPathComputeMs.length > 0) {
      computeSamples = persisted.hotPathComputeMs
    }
    sampleSource =
