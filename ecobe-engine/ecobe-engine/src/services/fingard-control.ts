...
export class FingardControlLayer {
  ...
  private async queryProvider(provider: string, region: string, timestamp: Date) {
    switch (provider) {
      ...
      case 'eia930': {
        // EIA-930 doesn't have getCurrentSignal, use getLatestSignal instead
        const latest = await eia930.getLatestSignal(region)
        return latest ? { carbonIntensity: latest.carbonIntensity, timestamp: latest.timestamp, isForecast: false } : null
      }
