...
const dashboardRootCandidates = [
  path.resolve(workspaceRoot, 'ecobe-dashboard'),
  path.resolve(repoRoot, 'ecobe-dashboard'),
  path.resolve(repoRoot, '..', 'ecobe-dashboard'),
]
const dashboardRoot = dashboardRootCandidates.find((candidate) => fs.existsSync(path.join(candidate, 'package.json'))) ?? null
...
const registryPath = dashboardRoot ? path.resolve(dashboardRoot, 'src', 'lib', 'claims', 'registry.json') : null
const publicSearchRoots = [
  ...(dashboardRoot
    ? [
        path.resolve(dashboardRoot, 'src', 'app'),
        path.resolve(dashboardRoot, 'src', 'components'),
      ]
    : []),
...
const fallbackClaims = [
  {
    id: 'proof_hash_per_decision',
    claim: 'Every decision carries a SHA-256 proof hash.',
    status: 'SAFE',
    evidenceSource: 'live_engine',
    requiredQualifier: null,
    allowedSurfaces: [],
  },
  {
    id: 'five_binding_actions',
    claim: 'CO2 Router returns five binding actions.',
    status: 'SAFE',
    evidenceSource: 'live_engine',
    requiredQualifier: null,
    allowedSurfaces: [],
  ...
  {
    id: 'replay_verified_decisions',
    claim: 'Replay verifies whether the engine still reaches the same governed result.',
    status: 'SAFE',
    evidenceSource: 'live_engine',
    requiredQualifier: null,
    allowedSurfaces: [],
...
async function main() {
  ...
  const engineRoot = fs.existsSync(path.resolve(repoRoot, 'prisma'))
    ? repoRoot
    : path.resolve(repoRoot, 'ecobe-engine')
  const prismaSchema = readText(path.resolve(engineRoot, 'prisma', 'schema.prisma'))
  const envConfig = readText(path.resolve(engineRoot, 'src', 'config', 'env.ts'))
  const eiaWorker = readText(path.resolve(engineRoot, 'src', 'workers', 'eia-ingestion.ts'))
  const ciRoute = readText(path.resolve(engineRoot, 'src', 'routes', 'ci.ts'))
  const modelCount = (prismaSchema.match(/^model\s+/gm) ?? []).length
  const migrationCount = fs.readdirSync(path.resolve(engineRoot, 'prisma', 'migrations'), { withFileTypes: true })
  ...
  const report = {
    ...
    engineUrl,
    dashboardUrl,
    registrySource: registryPath && fs.existsSync(registryPath) ? 'dashboard_registry' : 'fallback_runtime_registry',
    evidence: {
      slo: {
