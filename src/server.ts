...
async function start() {
  try {
    ...
    if (!waterArtifacts.healthy) {
      console.error(
        "Water artifact health check failed at startup:",
        waterArtifacts,
      );
      const recovery = recoverWaterArtifactsFromLastKnownGood();
      if (recovery.recovered) {
        console.warn(
          "Recovered water artifacts from last-known-good snapshot at startup",
        );
    ...
    if (
      verifierSink.status === "created" ||
      verifierSink.status === "updated"
    ) {
      console.log(
        `Decision event verifier sink ${verifierSink.status}: ${verifierSink.sinkId}`,
      );
    } else {
      console.warn(
        `Decision event verifier sink skipped: ${verifierSink.reason}`,
      );
    }
    ...
    server = app.listen(env.PORT, () => {
      ...
      console.log(
        `  Internal Health: http://localhost:${env.PORT}/internal/v1/health`,
      );
      console.log(
        `  Routing API: http://localhost:${env.PORT}/internal/v1/routing-decisions`,
      );
