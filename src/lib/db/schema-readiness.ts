...
const REQUIRED_ENUMS = [
  ...
  "OperatorRole",
  "DoctrineProposalStatus",
  "DoctrineVersionStatus",
  "DoctrineAuditEventType",
] as const;
...
async function schemaTableExists(schemaName: string, tableName: string) {
  const rows = (await prisma.$queryRawUnsafe(
...
export async function assertSchemaReadiness() {
  ...
  for (const table of REQUIRED_PGL_TABLES) {
    if (!(await schemaTableExists("pgl", table))) {
      failures.push(`Missing required PGL table: pgl.${table}`);
    }
  }
  ...
  if (await tableExists("CIDecision")) {
    ...
    const columns = new Set(
      columnRows.map((row: ColumnRow) => row.column_name),
    );
