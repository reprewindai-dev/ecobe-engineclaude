...
export class ProviderRouter {
  private static readonly LAST_KNOWN_GOOD_MAX_AGE_SEC = Math.max(
    env.GRID_SIGNAL_CACHE_TTL * 4,
    3600,
  );
  ...
  async getRoutingSignalRecord(
    ...
    const record = this.buildCachedRoutingSignalRecord(
      signal,
      timestamp,
      lastLatencyMs,
    );
    ...
    if (!record.degraded) {
      await GridSignalCache.cacheLastKnownGoodRoutingSignal(
        region,
        record,
      ).catch((error) => {
        console.warn(
          `Failed to cache last-known-good routing signal for ${region}:`,
          error,
        );
      });
      return record;
    ...
    const lastKnownGood =
      await GridSignalCache.getLastKnownGoodRoutingSignal(region);
    const lastKnownGoodSource =
      lastKnownGood?.signal.provenance.sourceUsed ?? "";
    const lastKnownGoodUsable = Boolean(
      lastKnownGood && !lastKnownGoodSource.includes("WATTTIME"),
    );
    const ageSec =
      ...
      (lastKnownGood?.fetchedAt
        ? Math.max(
            ...
            Math.round(
              (timestamp.getTime() -
                new Date(lastKnownGood.fetchedAt).getTime()) /
                1000,
            ),
          )
   ...
   */
  private async getLiveRoutingSignal(
    region: string,
    timestamp: Date,
  ): Promise<RoutingSignal> {
    ...
    if (wattTimeSignal) {
      ...
      return {
        ...
        provenance: {
          sourceUsed:
            wattTimeSignal.metadata?.signalType === "forecast_moer"
              ? "WATTTIME_MOER_FORECAST"
              : "WATTTIME_MOER",
        ...
        euOpenSignal.metadata?.zone === "EU-DE"
          ...
          : euOpenSignal.metadata?.zone === "EU-AT"
            ...
            : euOpenSignal.metadata?.zone === "EU-ES"
              ...
              : euOpenSignal.metadata?.zone === "EU-SE" ||
                  euOpenSignal.metadata?.zone === "EU-NO"
                ? "STATNETT_NORDIC_GENERATION_MIX_IPCC"
                : euOpenSignal.metadata?.zone === "EU-PT"
                  ? "PT_REN_DAILY_GENERATION_MIX_IPCC"
    ...
    if (fuelMixSignal !== null) {
      ...
      return {
        ...
        provenance: {
          ...
          disagreementPct: validation.disagreement.disagreementPct,
          validationNotes:
            `EIA-930 direct fuel mix: ${fuelMixSignal.carbonIntensity.toFixed(0)} gCO2/kWh (WattTime unavailable)` +
  ...
  private async getWattTimeSignal(
    region: string,
    timestamp: Date,
    ...
    try {
      ...
      if (currentMoer) {
        console.debug(
          `WattTime signal-index for ${region}/${wattTimeRegion}: ${currentMoer.moerPercent}`,
        );
      }
      ...
      const forecastMoer = await wattTime.getMOERForecast(
        wattTimeRegion,
        timestamp,
      );
      if (forecastMoer.length > 0) {
        ...
        return {
          ...
          confidence: 0.7,
          metadata: {
            signalType: "forecast_moer",
            forecastHorizon: forecast.timestamp,
          },
        };
      }
    } catch (error) {
   ...
   */
  private async getGBCarbonIntensitySignal(
    region: string,
  ): Promise<ProviderSignal | null> {
    ...
    try {
      ...
      if (current && current.intensity.forecast) {
        return {
          carbonIntensity:
            current.intensity.actual ?? current.intensity.forecast,
   ...
   */
  private async getDenmarkSignal(
    region: string,
  ): Promise<ProviderSignal | null> {
   ...
   */
  private async getFinlandSignal(
    region: string,
  ): Promise<ProviderSignal | null> {
    ...
    try {
      ...
      if (data) {
        return {
          ...
          metadata: {
            signalType: "fi_consumed_intensity",
            method: data.method,
          },
        };
      }
  ...
  private async getEuOpenSignal(
    ...
    try {
        ...
        zone === "EU-DE"
          ...
          : zone === "EU-AT"
            ...
            : zone === "EU-ES"
              ...
              : zone === "EU-SE" || zone === "EU-NO"
                ? await euOpenCarbon.getNordicIntensity(zone)
                : zone === "EU-PT"
                  ? await euOpenCarbon.getPortugalIntensity()
      ...
      return {
        ...
        timestamp: data.timestamp,
        estimatedFlag:
          data.method !== "smard-fuel-mix-ipcc" &&
          data.method !== "statnett-nordic-fuel-mix-ipcc" &&
          data.method !== "entsoe-generation-mix-ipcc",
        syntheticFlag: false,
  ...
  private fuelMixCICache = new Map<
    string,
    {
      result: {
        carbonIntensity: number;
   ...
   */
  private async getEia930FuelMixCI(
    region: string,
  ): Promise<{
    carbonIntensity: number;
    period: string;
    ...
    try {
      ...
      const computed =
        FuelMixParser.estimateCarbonIntensityFromEiaFuelTypeData(fuelMix);
  ...
  private async validateWithEmber(
    ...
    if (emberProfile?.structuralCarbonBaseline) {
      ...
      const isWattTimePercentile =
        disagreementSignals.length === 1 &&
        disagreementSignals[0]?.source === "watttime";
      // GB Carbon Intensity is authoritative real-time — its deviation from yearly baseline is expected
      const isAuthoritativeRealtime = disagreementSignals.some(
        (s) =>
          s.source === "gb_carbon_intensity" ||
          s.source === "electricity_maps" ||
          s.source === "dk_carbon" ||
          s.source === "fi_carbon" ||
          s.source === "ca_carbon" ||
          s.source === "eu_carbon" ||
          s.source === "eu_open_carbon" ||
          s.source === "eia930_fuel_mix",
      );
      ...
      if (isWattTimePercentile) {
        ...
        const deviation = Math.abs(
          emberProfile.structuralCarbonBaseline - signal.carbonIntensity,
        );
        const deviationPct =
          (deviation / Math.max(emberProfile.structuralCarbonBaseline, 1)) *
          100;
  ...
  private structuralProfileCache = new Map<
    string,
    { profile: any | null; expiry: number }
  >();
  ...
  async getStructuralProfile(region: string): Promise<any | null> {
    const entityCode =
      ProviderRouter.REGION_TO_EMBER_ENTITY[region] ?? region.toUpperCase();
    ...
    try {
      ...
      if (!profile || !profile.structuralCarbonBaseline) {
        this.structuralProfileCache.set(cacheKey, {
          profile: null,
          expiry: Date.now() + ProviderRouter.STRUCTURAL_CACHE_TTL,
        });
        return null;
      }
      ...
      console.warn(
        `Failed to get Ember structural profile for ${region}:`,
        error,
      );
    }
  ...
  private async fetchEmberStructuralData(
    entityCode: string,
  ): Promise<EmberData | null> {
    const [carbonIntensityYearly, demandYearly, capacityMonthly] =
      await Promise.all([
    ...
    const emberData: EmberData = {
      carbonIntensity: carbonIntensityYearly.map((point) => ({
        year: point.date,
        value: Number.isFinite(point.emissions_intensity_gco2_per_kwh)
          ? point.emissions_intensity_gco2_per_kwh
          : null,
      })),
  ...
  private computeSignalStalenessSec(
    ...
    return Math.max(
      0,
      Math.round((timestamp.getTime() - referenceTime) / 1000),
    );
  }
  ...
  private buildCachedRoutingSignalRecord(
    ...
    const degraded =
      signal.provenance.fallbackUsed ||
      signal.confidence < 0.6 ||
      signal.signalMode === "fallback";
  ...
  private buildConservativeLastKnownGoodRecord(
    ...
    const safetyMargin = Math.max(
      ...
      Math.round(
        record.signal.carbonIntensity *
          ProviderRouter.LAST_KNOWN_GOOD_SAFETY_MARGIN_FACTOR,
      ),
    );
    const adjustedSignal: RoutingSignal = {
      ...record.signal,
      carbonIntensity: Number(
        (record.signal.carbonIntensity + safetyMargin).toFixed(3),
      ),
      confidence: Math.max(
        0.05,
        Number((record.signal.confidence - 0.2).toFixed(3)),
      ),
      signalMode: "fallback",
      accountingMethod: "average",
      provenance: {
  ...
  async validateSignalQuality(signal: RoutingSignal): Promise<{
    ...
    if (
      signal.provenance.sourceUsed.includes("ESTIMATED") ||
    ...
    const meetsRequirements =
      qualityTier !== "low" || signal.provenance.fallbackUsed;
  ...
  async cacheRoutingSignal(
    ...
    const record =
      "signal" in signal
        ? signal
        : this.buildCachedRoutingSignalRecord(signal, timestamp, null);

    await GridSignalCache.cacheRoutingSignal(
      region,
   ...
   */
  async getCachedRoutingSignalRecord(
    region: string,
    timestamp: Date,
  ): Promise<CachedRoutingSignalRecord | null> {
    const cachedRecord = await GridSignalCache.getCachedRoutingSignal(
      region,
      timestamp.toISOString(),
    );
    if (cachedRecord) {
      return {
        ...cachedRecord,
        stalenessSec:
          cachedRecord.stalenessSec ??
          this.computeSignalStalenessSec(cachedRecord.signal, timestamp),
      };
    }

    const cachedDisagreement =
      await GridSignalCache.getCachedProviderDisagreement(
        region,
        timestamp.toISOString(),
      );
      ...
      const reconstructed: CachedRoutingSignalRecord = {
        signal: {
          ...
          provenance: {
            ...
            validationNotes:
              "Reconstructed from cached provider disagreement only",
          },
        },
        fetchedAt: new Date().toISOString(),
        stalenessSec: 0,
  ...
  async getCachedRoutingSignal(
    region: string,
    timestamp: Date,
  ): Promise<RoutingSignal | null> {
  ...
  async recordSignalProvenance(
    ...
    await GridSignalAudit.recordRoutingDecision(
      ...
      signal.source === "watttime" ? "US" : signal.source, // Simplified region mapping
      {
        ...
        demandRampPct:
          typeof (signal as any).metadata?.demandRampPct === "number"
            ? (signal as any).metadata.demandRampPct
            : null,
