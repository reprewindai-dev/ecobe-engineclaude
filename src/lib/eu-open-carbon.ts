...
export interface EuOpenCarbonData {
  ...
  method:
    | "smard-fuel-mix-ipcc"
    | "ree-daily-generation-mix-ipcc"
    | "statnett-nordic-fuel-mix-ipcc"
    | "ren-daily-generation-mix-ipcc"
    | "entsoe-generation-mix-ipcc";
...
const STATNETT_MAX_STALENESS_MS = 3 * 60 * 60 * 1000;
const REN_DAILY_MAX_STALENESS_MS = 48 * 60 * 60 * 1000;
...
function normalizeReeFuel(title: unknown): string {
  ...
  if (
    ...
    value.includes("fuel") ||
    value.includes("fioul")
  )
    return "oil";
  if (
    value.includes("turbina de gas") ||
    value.includes("ciclo combinado") ||
    value === "gas"
  )
    return "gas";
...
function parseStatnettNumber(value: unknown): number {
...
function extractCountryValue(
  ...
  const suffix = `${countryCode}Desc`;
  const item = (items ?? []).find((entry) =>
    String(entry?.titleTranslationId ?? "").endsWith(suffix),
  );
  if (!item) return null;

  const value = parseStatnettNumber(item.value);
  const timestamp = String(item.measuredAt ?? "");
  if (value <= 0 || !timestamp) return null;
...
function normalizeRenFuel(label: string): string {
  const value = label
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
  if (
    value.includes("hidrica") ||
    value.includes("hydro") ||
    value.includes("water")
  )
    return "hydro";
  if (value.includes("eolica") || value.includes("wind")) return "wind";
  if (value.includes("solar")) return "solar";
  if (value.includes("biomassa") || value.includes("biomass")) return "biomass";
  if (
    value.includes("gas natural") ||
    value.includes("natural gas") ||
    value.includes("ciclo combinado")
  )
    return "gas";
  if (value.includes("carvao") || value.includes("coal")) return "coal";
  if (
    value.includes("fuel") ||
    value.includes("diesel") ||
    value.includes("oil")
    ...
    return "oil";
  if (value.includes("residu") || value.includes("waste")) return "waste";
  if (value.includes("import")) return "ignore";
  if (value.includes("bombagem") || value.includes("pumping")) return "ignore";
  if (value.includes("consumo") || value.includes("consumption"))
    return "ignore";
  return "other";
}

function collectRenFuelValues(input: unknown, fuels: FuelBreakdown) {
  if (!input || typeof input !== "object") return;

  if (Array.isArray(input)) {
    for (const item of input) collectRenFuelValues(item, fuels);
    return;
  }
  ...
  const possibleValue = [
    ...
    record.y,
    record.Y,
  ].find((value) => Number.isFinite(Number(String(value).replace(",", "."))));

  if (typeof possibleLabel === "string" && possibleValue !== undefined) {
    const fuel = normalizeRenFuel(possibleLabel);
    if (fuel !== "ignore")
      addFuel(fuels, fuel, numeric(String(possibleValue).replace(",", ".")));
  }

  for (const value of Object.values(record)) {
    if (value && typeof value === "object") collectRenFuelValues(value, fuels);
  }
}
...
export class EuOpenCarbonClient {
  ...
  private async logSuccess(source: string) {
    try {
      await recordIntegrationSuccess(source as any);
    } catch {
      /* ignore */
    }
  }
  ...
  private async logFailure(source: string, message: string) {
    try {
      await recordIntegrationFailure(source as any, message);
    } catch {
      /* ignore */
    }
  }
  ...
  async getSpainIntensity(): Promise<EuOpenCarbonData | null> {
    ...
    try {
      ...
      if (
        !latestTimestamp ||
        !isFreshEnough(latestTimestamp, REE_DAILY_MAX_STALENESS_MS)
      ) {
        await this.logFailure(
          "ES_REE_OPEN_DATA",
          `REE daily generation mix is stale or empty: ${latestTimestamp || "none"}`,
        );
        return null;
      }
      ...
      if (carbonIntensity == null) {
        await this.logFailure(
          "ES_REE_OPEN_DATA",
          "REE generation mix returned no usable fuel breakdown",
        );
        return null;
      }
      ...
      await this.logFailure(
        "ES_REE_OPEN_DATA",
        error?.message ?? "Spain REE carbon fetch failed",
      );
  ...
  async getNordicIntensity(
    zone: "EU-SE" | "EU-NO",
  ): Promise<EuOpenCarbonData | null> {
    const countryCode = zone === "EU-SE" ? "SE" : "NO";

    try {
      const response = await axios.get<Record<string, any>>(
        STATNETT_NORDIC_OVERVIEW_URL,
        {
          timeout: 12000,
        },
      );
      ...
      const fuelMap: Array<{ key: string; fuel: string }> = [
        { key: "NuclearData", fuel: "nuclear" },
        { key: "HydroData", fuel: "hydro" },
        { key: "ThermalData", fuel: "otherConventional" },
        { key: "WindData", fuel: "wind" },
        { key: "NotSpecifiedData", fuel: "other" },
      ];

      for (const { key, fuel } of fuelMap) {
        const point = extractCountryValue(response.data[key], countryCode);
        if (!point) continue;
        addFuel(fuelBreakdownMw, fuel, point.value);
        timestamps.push(point.timestamp);
      }

      const timestamp = latestTimestamp(timestamps);
      if (!timestamp || !isFreshEnough(timestamp, STATNETT_MAX_STALENESS_MS)) {
        await this.logFailure(
          "STATNETT_NORDIC_OPEN_DATA",
          `Statnett Nordic generation mix is stale or empty for ${zone}`,
        );
        return null;
      }
      ...
      if (carbonIntensity == null) {
        await this.logFailure(
          "STATNETT_NORDIC_OPEN_DATA",
          `Statnett Nordic generation mix has no usable fuels for ${zone}`,
        );
        return null;
      }

      await this.logSuccess("STATNETT_NORDIC_OPEN_DATA");
      return {
        zone,
        carbonIntensity,
        timestamp,
        isForecast: false,
        method: "statnett-nordic-fuel-mix-ipcc",
        fuelBreakdownMw,
        sourceUrl: STATNETT_NORDIC_OVERVIEW_URL,
        sourceFreshness: "hourly",
      };
    } catch (error: any) {
      await this.logFailure(
        "STATNETT_NORDIC_OPEN_DATA",
        error?.message ?? `Statnett Nordic fetch failed for ${zone}`,
      );
      console.warn(
        `Statnett Nordic carbon signal failed for ${zone}:`,
        error?.message ?? error,
      );
      return null;
    }
  ...
  async getPortugalIntensity(): Promise<EuOpenCarbonData | null> {
    const now = new Date();
    ...
    for (let offsetDays = 0; offsetDays <= 3; offsetDays += 1) {
      ...
      try {
        const response = await axios.get<unknown>(
          REN_PRODUCTION_BREAKDOWN_URL,
          {
            params: { culture: "en-US", date: dateParam },
            timeout: 15000,
          },
        );

        const fuelBreakdownMw: FuelBreakdown = {};
        collectRenFuelValues(response.data, fuelBreakdownMw);
        const carbonIntensity = weightedIntensity(fuelBreakdownMw);
        if (carbonIntensity == null) continue;

        const timestamp = `${dateParam}T12:00:00.000Z`;
        if (!isFreshEnough(timestamp, REN_DAILY_MAX_STALENESS_MS)) continue;

        await this.logSuccess("PT_REN_OPEN_DATA");
        return {
          zone: "EU-PT",
          carbonIntensity,
          timestamp,
          isForecast: false,
          method: "ren-daily-generation-mix-ipcc",
          fuelBreakdownMw,
          sourceUrl: REN_PRODUCTION_BREAKDOWN_URL,
          sourceFreshness: "daily",
        };
      } catch (error: any) {
        if (offsetDays === 3) {
          await this.logFailure(
            "PT_REN_OPEN_DATA",
            error?.message ?? "REN Portugal carbon fetch failed",
          );
          console.warn(
            "REN Portugal carbon signal failed:",
            error?.message ?? error,
          );
        }
      }
    ...
    await this.logFailure(
      "PT_REN_OPEN_DATA",
      "REN Portugal generation mix is stale, empty, or unreachable",
    );
    return null;
  }

  async getEntsoeIntensity(
    zone: EuOpenCarbonZone,
  ): Promise<EuOpenCarbonData | null> {
    ...
    try {
      ...
      if (
        !parsed ||
        !isFreshEnough(parsed.timestamp, ENTSOE_MAX_STALENESS_MS)
      ) {
        await this.logFailure(
          "ENTSOE_TRANSPARENCY",
          `ENTSO-E generation mix is stale or empty for ${zone}`,
        );
        return null;
      }
      ...
      if (carbonIntensity == null) {
        await this.logFailure(
          "ENTSOE_TRANSPARENCY",
          `ENTSO-E generation mix has no usable fuels for ${zone}`,
        );
        return null;
      }
      ...
      await this.logFailure(
        "ENTSOE_TRANSPARENCY",
        error?.message ?? `ENTSO-E fetch failed for ${zone}`,
      );
      console.warn(
        `ENTSO-E carbon signal failed for ${zone}:`,
        error?.message ?? error,
      );
      return null;
    }
  ...
  private async getSmardIntensity(
    zone: "EU-DE" | "EU-AT",
    smardRegion: "DE" | "AT",
  ): Promise<EuOpenCarbonData | null> {
    const source =
      zone === "EU-DE" ? "DE_SMARD_OPEN_DATA" : "AT_SMARD_OPEN_DATA";
    ...
    try {
      ...
      const latestTimestamp =
        latestTimestampMs > 0 ? new Date(latestTimestampMs).toISOString() : "";
      if (
        !latestTimestamp ||
        !isFreshEnough(latestTimestamp, SMARD_MAX_STALENESS_MS)
      ) {
        await this.logFailure(
          source,
          `SMARD generation mix is stale or empty for ${smardRegion}: ${latestTimestamp || "none"}`,
        );
        return null;
      }
      ...
      if (carbonIntensity == null) {
        await this.logFailure(
          source,
          `SMARD generation mix has no usable fuel data for ${smardRegion}`,
        );
        return null;
      }
      ...
      await this.logFailure(
        source,
        error?.message ?? `SMARD fetch failed for ${smardRegion}`,
      );
      console.warn(
        `SMARD carbon signal failed for ${smardRegion}:`,
        error?.message ?? error,
      );
      return null;
    }
  ...
  private async getLatestSmardPoint(
    ...
    try {
      ...
      const index = await axios.get<{ timestamps?: number[] }>(indexUrl, {
        timeout: 8000,
      });
      const timestamp =
        index.data.timestamps?.[index.data.timestamps.length - 1];
      ...
      }>(seriesUrl, { timeout: 8000 });
      const latest = [...(series.data.series ?? [])]
        .reverse()
        .find(
          ([pointTimestamp, value]) =>
            Number.isFinite(pointTimestamp) && Number.isFinite(value),
        );
  ...
  private parseEntsoeGenerationMix(
    ...
    const timeSeriesBlocks =
      xml.match(/<TimeSeries[\s\S]*?<\/TimeSeries>/g) ?? [];
    ...
    for (const block of timeSeriesBlocks) {
      ...
      for (const pointBlock of pointBlocks) {
        ...
        const pointTime = addMinutes(
          startDate,
          (position - 1) * intervalMinutes,
        );
    ...
    for (const [fuel, point] of Object.entries(latestByFuel)) {
      ...
      if (
        !latestTimestamp ||
        point.timestamp.getTime() > latestTimestamp.getTime()
      ) {
        latestTimestamp = point.timestamp;
      }
    }

    if (!latestTimestamp || Object.keys(fuelBreakdownMw).length === 0)
      return null;
    return {
